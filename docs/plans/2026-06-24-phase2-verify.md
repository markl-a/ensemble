...
- 若某些 tailnet peer 不應納入本次個人 fleet（例如公司機、臨時禁用機、或 operator 明確排除的節點），在未追蹤的 manifest 加 `forbidden_nodes: ["<node>"]`；`phase2-goal-shape.ps1` 會拒絕任何 `nodes`、`conductor`、main route 或 satellite 使用該節點，避免為了湊 5-node 誤把禁用機器拉進驗收。
- 衛星專案的 `satellites[].node` 是該專案的執行/歸屬節點；預設會同時產生 codex+claude 都指向該節點。若某台衛星的特定 CLI 未登入或不適合跑 reviewer，可加 `satellites[].routes.codex` / `satellites[].routes.claude` 分別覆寫 generated crew 的 agent route；這些 route 一樣必須指向 manifest.nodes，且不可使用 `forbidden_nodes`。
- 主專案的 `main.routes.<agent>` 若指向 `conductor`，generated crew 會省略該 agent 的 `node`，讓它使用 conductor 本機 CLI；只有指向其他 manifest node 時才生成遠端 URL。這避免 conductor 自己的主專案 CLI 也走大型 remote git-sync。
