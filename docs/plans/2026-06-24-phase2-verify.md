...
5. 確認 plan 正確後，該節點直接跑被選中的任務並自動收斂 evidence：`pwsh scripts\phase2-fleet.ps1 -Manifest phase2-fleet.local.json -Node <m1..m5> -Materialize -RunSelected -VerifyEvidence`（`-RunSelected` 必須指定非 `all` 的 `-Node`）
