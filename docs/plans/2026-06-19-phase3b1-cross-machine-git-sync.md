...
- **Prune applied refs (gate follow-up, claude):** each remote run leaves a `refs/ensemble/dispatch/<job_id>` ref in the orchestrator repo forever; delete the temp ref after the ff-merge succeeds (the worktree branch already carries the commit).
- **No-op node bundle (gate follow-up, claude):** `commit_all`'s `Ok(false)` is ignored on the node, so a no-edit agent still returns an (empty) bundle + success; the orchestrator's `(Some, None)` "no edits" guard therefore only fires for a malformed node, not a genuine no-op. Acceptable for slice-1.
