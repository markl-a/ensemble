...
- 2026-06-26 — **Deferred minor (a) done: content-stable FNV-1a `dispatch::task_id`.** Replaced
  `DefaultHasher` (only stable within a binary build) with a FIXED FNV-1a 64-bit hash (same constants
  as the phantom-mesh fleet `task_id`), so the durable ledger PRIMARY KEY can't drift on a Rust
  toolchain upgrade and silently re-run an already-drained batch. Golden + property test added; WSL
  `cargo test --lib dispatch` green; codex + claude LGTM. Landed on `phase2-verify-fixes` @adf3deb.
- 2026-06-26 — **Phase 2 service port is now fleet-configurable; Phantom can coexist on 7878.**
  Real deployment found one Windows peer's `:7878` was already occupied by Phantom, so `ensemble serve`
  installed successfully but exited immediately and `ensemble nodes` correctly saw no ensemble host.
  Added `serve/up/mesh/nodes --port <n>`, persisted `serve --install-service --port <n>` across
  Windows scheduled tasks / launchd / systemd service renderers, and taught `phase2-fleet.ps1`
  to read `service.port` from the manifest. The same port now drives generated crew node URLs,
  service bootstrap commands, and `-CheckNodes` mesh/nodes checks. Verified with focused Rust
  tests, `phase2-fleet.ps1 -SelfTest`, release build/install, and a real Windows peer service install:
  Phantom still answers on `:7878`, while ensemble answers `/health` on `:8788` and
  `ensemble mesh --port 8788` / `ensemble nodes --port 8788` discover the peer. Fleet remains
  partial because the other three peers still need SSH/service bootstrap.
...
- 2026-06-25 — **Phase 2 fleet acceptance reports now bind generated crew and project spec hashes.**
  ...
  manifest-derived generated crew SHA-256 plus a selected-project spec SHA-256. `-VerifyReports`
  compares those hashes against the current manifest-generated crew and project metadata, so a
  report from an older task, route, or quorum plan cannot satisfy Slice C after the manifest
  changes. Verified with `phase2-fleet.ps1 -SelfTest`.
